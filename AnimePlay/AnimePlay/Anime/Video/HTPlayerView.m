...
#import "HTFullViewController.h"
#import "HTPlayerTitleView.h"
#import "HTPlayerToolView.h"
#import "HTPlayerFailedView.h"
#import "HTPlayerLayerView.h"

@interface HTPlayerView () <HTPlayerToolViewDelegate, HTPlayerTitleViewDelegate, HTPlayerFailedViewDelegate>

@property (nonatomic, strong) AVPlayer * player;
@property (nonatomic, strong) AVPlayerItem * playerItem;
@property (nonatomic, strong) AVPlayerLayer * playerLayer;

@property (nonatomic, strong) HTFullViewController * fullVC;
@property (nonatomic, weak) UIViewController * currentVC;

@property (nonatomic, strong) HTPlayerTitleView * titleView;
@property (nonatomic, strong) HTPlayerToolView * toolView;
@property (nonatomic, strong) HTPlayerFailedView * failedView;
@property (nonatomic, strong) HTPlayerLayerView * layerView;
...
@property (nonatomic, strong) HTVideoModel * videoModel;
...
- (void)setVideoModels:(NSArray<HTVideoModel *> *)videoModels playVideoId:(NSString *)videoId {
    ...
    if (videoId.length > 0) {
        ...
        for (HTVideoModel * model in self.videoArr) {
...
- (void)videoPlayEnd {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        [self->_toolView mas_updateConstraints:^(MASConstraintMaker *make) {
            
            make.bottom.mas_equalTo(@0);
        }];
        [self->_titleView mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        if (playerItem.status == AVPlayerItemStatusReadyToPlay) {
            ...
            [self.player seekToTime:seekTime completionHandler:^(BOOL finished) {
                ...
                if (finished) {
                    ...
                    self->_toolView.currentTimeLabel.text = [self convertTimeToString:current];
                }
            }];
...
- (void)setPlayer {
    ...
    if (self.videoModel) {
        ...
        if (self.videoModel.url) {
            ...
            [_layerView ht_addPlayerLayer:self.playerLayer];
...
//切换当前播放的内容
- (void)replaceCurrentPlayerItemWithVideoModel:(HTVideoModel *)model {
...
- (void)progressValueChange:(HTProgressSlider *)slider {
...
// 重新播放
- (void)failedViewDidReplay:(HTPlayerFailedView *)failedView {
...
- (void)titleViewDidExitFullScreen:(HTPlayerTitleView *)titleView {
...
- (void)toolView:(HTPlayerToolView *)toolView fullScreen:(BOOL)isFull {
    ...
    if (isFull) {
        ...
        [_currentVC presentViewController:self.fullVC animated:NO completion:^{
            ...
            [self->_titleView ht_showBackButton];
...
- (void)showOHTideBar {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        [self->_toolView mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.mas_equalTo(@(self->_isShowToolView ? 44 : 0));
        }];
        [self->_titleView mas_updateConstraints:^(MASConstraintMaker *make) {
           
            make.top.mas_equalTo(@(self->_isShowToolView ? -44 : 0));
        }];
        [self layoutIfNeeded];
    } completion:^(BOOL finished) {
        
        self->_isShowToolView = !self->_isShowToolView;
        if (self->_isShowToolView) {
...
- (HTFullViewController *)fullVC {
    
    if (!_fullVC) {
        
        HTFullViewController * fullVC = [[HTFullViewController alloc] init];
        _fullVC = fullVC;
    }
    return _fullVC;
}

- (HTPlayerLayerView *)layerView {
    
    if (!_layerView) {
        
        HTPlayerLayerView * layerView = [[HTPlayerLayerView alloc] init];
...
- (HTPlayerFailedView *)failedView {
    
    if (!_failedView) {
        
        HTPlayerFailedView * failedView = [[HTPlayerFailedView alloc] init];
...
- (HTPlayerToolView *)toolView {
    
    if (!_toolView) {
        
        HTPlayerToolView * toolView = [[HTPlayerToolView alloc] init];
...
- (HTPlayerTitleView *)titleView {
    
    if (!_titleView) {
        
        HTPlayerTitleView * titleView = [[HTPlayerTitleView alloc] init];
